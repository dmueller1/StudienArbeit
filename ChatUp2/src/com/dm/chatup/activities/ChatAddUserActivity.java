...
import com.dm.chatup.internet.Network.AddUserToChat;
...
public class ChatAddUserActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        List<Contact> allContacts = mySystem.getMyContacts();
        Chat aktChat = mySystem.getChatFromID(mySystem.getOpenChat());
		...
		contactList.setAdapter(new UserArrayAdapter(this,
				R.layout.listitem_contact, mySystem.getUserID(), filteredContacts));
    ...
    private class ATAddUsersToChat extends AsyncTask<int[], Boolean, Void> {
    	...
    	int chatID = AppSystem.getInstance().getOpenChat();
		...
		protected Void doInBackground(int[]... params) {
			...
			for(int i = 0; i < userIDsToAdd.length; i++) {
				if (AppSystemOnline.getInstance().addUserToChat(chatID, userIDsToAdd[i]) == false) {
					this.cancel(true);
				} 
			}
			return null;
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if (keyCode == KeyEvent.KEYCODE_BACK) {
            ...
            i.putExtra("chatID", AppSystem.getInstance().getOpenChat());
            startActivity(i);
            return true;
        }
